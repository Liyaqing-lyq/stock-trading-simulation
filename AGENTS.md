...
1. `bash scripts/verify.sh --with-gui` 全绿——这一条命令 = 测试套件（35 项）+ 从 CSV 的黑盒复算 + 界面真实启动；
   失败必须定位根因后修复，不允许删测试或放宽断言掩盖；
2. 与评分表对应的功能项在 `docs/01-需求与评分拆解.md` 中更新状态；
3. 真实运行输出（而非推测结果）追加到 `docs/05-运行与验证记录.md`。
